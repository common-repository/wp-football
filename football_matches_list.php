<?php
	...
	$matches = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS * FROM {$table_prefix}fb_match WHERE fb_match_id_league = '$id_league' ORDER BY fb_match_id_phase, fb_match_id_group {$limit_by}");
			...
			<table class="widefat"> 
				...
				foreach ( $matches as $match ) {
					...
					echo '
						  ...
						  <td><a href="'.$path.'?page=wp-football/football-matches.php&amp;action=edit&amp;new=0&amp;id_match='.$match->fb_match_id.'&amp;id_league='.$match->fb_match_id_league.'&amp;paged='.$_GET['paged'].'"><img src="'.$editar_icon.'" alt="'.__("Change Team").'" title="'.__("Change Team").'" /></a></td>
					  </tr>';  
					  $i++;
				}
				?>
