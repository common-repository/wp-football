<?php
	...
	if (!$id_phase) $id_phase = $_GET['id_phase'];
	if ( ! isset( $_GET['paged'] ) || $_GET['paged'] < 1 ) $_GET['paged'] = 1;
	
	$start = ( $_GET['paged'] - 1 ) * 15;
	$limit = 15;
	$limit_by  = 'LIMIT ' . intval($start) . ',' . intval($limit);
	$matches = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS * FROM {$table_prefix}fb_match WHERE fb_match_id_league = '$id_league' AND fb_match_id_phase = '$id_phase' ORDER BY fb_match_number {$limit_by}");
	...
	$page_links = paginate_links( array(
		'base' => add_query_arg( 'paged', '%#%', $path."?page=wp-football/football-matches.php&amp;action=edit&amp;new=1&amp;id_league=".$id_league ),
		'format' => '',
		'prev_text' => __('&laquo;'),
		'next_text' => __('&raquo;'),
...
?>
	<div class="wrap">	
		...
		<form id="manage_eventos_form" method="post" action="<?php echo $path ?>?page=wp-football/football-matches.php&amp;action=delete&amp;id_league=<?php echo $id_league; ?>" onsubmit="javascript:check=confirm( '<?php _e('Excluding this(these) Match(es) ?', 'wp-football'); ?>');if(check==false) return false;"> 
			<div class="tablenav">
				<div class="alignleft">
			...
			<table class="widefat"> 
				...
				foreach ( $matches as $match ) {
					...
					echo '
						  <th scope="row" class="check-column"><input type="checkbox" name="del_'.$match->fb_match_id.'" /></th>
						  ...
						  <td><a href="'.$path.'?page=wp-football/football-update_phase.php&amp;action=edit&amp;new=0&amp;id_match='.$match->fb_match_id.'&amp;id_league='.$match->fb_match_id_league.'&amp;paged='.$_GET['paged'].'"><img src="'.$editar_icon.'" alt="'.__("Change Team").'" title="'.__("Change Team").'" /></a></td>
					  </tr>';  
					  $i++;
				}
				?>
